...
int main(int argc, char* argv[]) {
	...
	cudaMalloc((void**)&dev_a, (N>>1) * sizeof(int));
stopwatch_start (timerA1);
	cudaMalloc((void**)&dev_r, (N>>1) * sizeof(int));
	cudaMalloc((void**)&dev_g, (N>>1) * sizeof(int));
	cudaMalloc((void**)&dev_b, (N>>1) * sizeof(int));
	cudaMalloc((void**)&dev_c, 3 * (N + 5120) * sizeof(float));


	cudaMemcpy(dev_a, shen, (N>>1) * sizeof(int), cudaMemcpyHostToDevice);
	cudaMemcpy(dev_r, red,  (N>>1) * sizeof(int), cudaMemcpyHostToDevice);
	cudaMemcpy(dev_g, green, (N>>1) * sizeof(int), cudaMemcpyHostToDevice);
	cudaMemcpy(dev_b, blue, (N>>1) * sizeof(int), cudaMemcpyHostToDevice);
	cudaMemcpy(dev_c, c, (N + 5120) * 3 * sizeof(float), cudaMemcpyHostToDevice);
	...
	cudaMemcpy(f, dev_c, 3 * (N + 5120) * sizeof(float), cudaMemcpyDeviceToHost);	
	...
	commA2 = stopwatch_stop (timerA3) + commA1;
        ...
        cudaMalloc((void**)&dev_a2, (N>>1) * sizeof(int));
        cudaMalloc((void**)&dev_r2, (N>>1) * sizeof(int));
        cudaMalloc((void**)&dev_g2, (N>>1) * sizeof(int));
        cudaMalloc((void**)&dev_b2, (N>>1) * sizeof(int));
        cudaMalloc((void**)&dev_c2, 3 * (N + 5120) * sizeof(float));


        cudaMemcpy(dev_a2, shen+(N>>1), (N>>1) * sizeof(int), cudaMemcpyHostToDevice);
        cudaMemcpy(dev_r2, red+(N>>1),  (N>>1) * sizeof(int), cudaMemcpyHostToDevice);
        cudaMemcpy(dev_g2, green+(N>>1), (N>>1) * sizeof(int), cudaMemcpyHostToDevice);
        cudaMemcpy(dev_b2, blue+(N>>1), (N>>1) * sizeof(int), cudaMemcpyHostToDevice);
        cudaMemcpy(dev_c2, c+(N + 5120) * 3, (N + 5120) * 3 * sizeof(float), cudaMemcpyHostToDevice);

        //dim3 grid(DIM,DIM);
        add <<< blocks, threads >>>(dev_a2, dev_r2, dev_g2, dev_b2, dev_c2);
        cudaMemcpy(f+(N + 5120) * 3, dev_c2, 3 * (N + 5120) * sizeof(float), cudaMemcpyDeviceToHost);
